...
**注**: 本書で列挙する SPDX 識別子（MIT / Apache-2.0 / CC0 / CC-BY / OFL / ISC、CC-BY は主にアニメーション/効果音向け）はドキュメント上の表記です。MCP 応答スキーマ（`LicenseInfoSchema`）上は `free-commercial` または `apple-system` に正規化されます。スキーマ拡張は別 PR で扱います。
...
**例外**: SF Symbols は Apple のシステムフォントとして名前ベース参照するため、SVG 配布要件を免除します。スキーマ上は `license: apple-system` として扱います。
